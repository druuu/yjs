...
utils.EventListenerHandler = EventListenerHandler
...
class EventHandler extends EventListenerHandler {
  ...
  */
  awaitAndPrematurelyCall (ops) {
    this.awaiting++
    ops.map(utils.copyOperation).forEach(this.onevent)
  }
      ...
      while (array.length > 0) {
        for (var i = 0; i < array.length; i++) {
          ...
          for (var j = 0; j < array.length; j++) {
            if (utils.matchesId(array[j], array[i].left)) {
    ...
    if (this.awaiting === 0 && this.waiting.length > 0) {
      ...
      for (let i = 0; i < this.waiting.length; i++) {
        ...
        if (o.struct === 'Insert') {
          ...
          if (!utils.compareIds(_o.id, o.id)) {
            // o got extended
            ...
            o.left = left != null ? utils.getLastId(left) : null
          }
        }
      }
    ...
    for (var oid = 0; oid < ops.length; oid++) {
      ...
      if (op.struct === 'Insert') {
        for (var i = this.waiting.length - 1; i >= 0; i--) {
          ...
          if (w.struct === 'Insert') {
            if (utils.matchesId(w, op.left)) {
              // include the effect of op in w
              w.right = op.id
              // exclude the effect of w in op
              op.left = w.left
            } else if (utils.compareIds(w.id, op.right)) {
              // similar..
              w.left = utils.getLastId(op)
              op.right = w.right
            }
          }
        }
      ...
      if (del.struct === 'Delete') {
        if (newLeft != null) {
          for (var i = 0; i < this.waiting.length; i++) {
            ...
            if (w.struct === 'Insert' && utils.compareIds(del.target, w.left)) {
              w.left = newLeft
            }
          }
        }
...
utils.CustomType = CustomType

utils.isTypeDefinition = function isTypeDefinition (v) {
  if (v != null) {
    if (v instanceof utils.CustomType) return [v]
    else if (v.constructor === Array && v[0] instanceof utils.CustomType) return v
    else if (v instanceof Function && v.typeDefinition instanceof utils.CustomType) return [v.typeDefinition]
  }
  return false
}
...
utils.copyOperation = copyOperation
...
utils.createSmallLookupBuffer = createSmallLookupBuffer
