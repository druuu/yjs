...
class TransactionInterface {
  ...
  * createType (typedefinition, id) {
    ...
    if (id[0] === '_') {
      ...
      op = struct[structname].create(id)
  ...
  /* createType (typedefinition, id) {
    var structname = typedefinition[0].struct
    id = id || this.store.getNextOpId(1)
    var op = struct[structname].create(id)
    op.type = typedefinition[0].name
    if (typedefinition[0].appendAdditionalInfo != null) {
      yield* typedefinition[0].appendAdditionalInfo.call(this, op, typedefinition[1])
    }
  ...
  * markGarbageCollected (id, len) {
    ...
    if (
      prev != null &&
      prev.gc &&
      utils.compareIds([prev.id[0], prev.id[1] + prev.len], n.id)
    ) {
      prev.len += n.len
    ...
    if (
      next != null &&
      next.gc &&
      utils.compareIds([n.id[0], n.id[1] + n.len], next.id)
    ) {
      n.len += next.len
    ...
    if (o != null) {
        ...
        if (o.originOf != null && o.originOf.length > 0) {
          ...
          if (right.right != null) {
            ...
            while (ids.some(function (id) {
              return utils.compareIds(id, i.origin)
            })) {
              if (utils.compareIds(i.origin, o.id)) {
                // reset origin of i
                i.origin = neworigin
      ...
      if (o.origin != null) {
        ...
        origin.originOf = origin.originOf.filter(function (_id) {
          return !utils.compareIds(id, _id)
        })
      ...
      if (parent != null) {
        ...
        if (o.parentSub != null) {
          if (utils.compareIds(parent.map[o.parentSub], o.id)) {
            setParent = true
            if (o.right != null) {
          ...
          if (utils.compareIds(parent.start, o.id)) {
            // gc'd op is the start
            setParent = true
            parent.start = o.right
          }
          if (utils.matchesId(o, parent.end)) {
            // gc'd op is the end
            setParent = true
            parent.end = o.left
          }
  ...
  * tryCombineWithLeft (op) {
    if (
      ...
      op.content != null &&
      op.left[0] === op.id[0] &&
      utils.compareIds(op.left, op.origin)
    ) {
  ...
  * getInsertionCleanStart (id) {
    ...
    if (ins != null) {
      if (ins.id[1] === id[1]) {
        ...
        var left = utils.copyObject(ins)
        ins.content = left.content.splice(id[1] - ins.id[1])
        ins.id = id
        var leftLid = utils.getLastId(left)
        ins.origin = leftLid
  ...
  * getOperation (id/* :any */)/* :Transaction<any> */ {
    ...
    if (id[0] !== '_' || o != null) {
      ...
      if (comp.length > 1) {
        var struct = comp[0]
        var op = struct[struct].create(id)
        op.type = comp[1]
        yield* this.setOperation(op)
        return op
      } else {
        ...
        if (op.struct !== 'Insert') {
          ...
          while (true) {
            if (o.left == null) {
              op.left = null
              send.push(op)
              if (!utils.compareIds(o.id, op.id)) {
                o = struct[op.struct].encode(o)
            ...
            while (missing_origins.length > 0 && utils.matchesId(o, missing_origins[missing_origins.length - 1].origin)) {
              missing_origins.pop()
            }
            if (o.id[1] < (startSS[o.id[0]] || 0)) {
              // case 2. o is known
              op.left = utils.getLastId(o)
              send.push(op)
              break
            } else if (utils.matchesId(o, op.origin)) {
              // case 3. o is op.origin
              op.left = op.origin
              send.push(op)
              op = struct[op.struct].encode(o)
              op.right = newright
              ...
              var s = struct[op.struct].encode(o)
  ...
  * makeOperationReady (startSS, op) {
    op = struct[op.struct].encode(op)
    op = utils.copyObject(op) -- use copyoperation instead now!
    var o = op
    var ids = [op.id]
    ...
    while (o.right != null) {
      ...
      if (o.right[1] < (startSS[o.right[0]] || 0) || !ids.some(function (id) {
        return utils.compareIds(id, right.origin)
      })) {
        break
      }
      ids.push(o.right)
